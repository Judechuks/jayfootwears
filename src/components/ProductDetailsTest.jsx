...
*/
  let tempShoeSizeArray = [];
  let tempShoeSizeArray2 = [];
  const sendToCart = (obj) => {
    const initialCart = cartItems.filter((item) => item?.name !== obj.name);
    // const initialCart = cartItems.filter((item) => item?.name === obj.name && item?.size === obj.size);

    const alreadyInCart = cartItems.find((item) => item?.name === obj.name);
    // const alreadyInCart = cartItems.find((item) => item?.name === obj.name && item?.size === obj.size);

    if (alreadyInCart) {
      console.log("already in Cart");
      if (alreadyInCart.size === obj.size) {
        setCartItems([
          ...initialCart,
          {
            ...alreadyInCart,
        ...
        setCartItems([
          ...initialCart,
          {
            ...alreadyInCart,
            quantity: alreadyInCart.quantity + 1,
            size: alreadyInCart.size + ", " + obj.size,
          },
        ]);
      }
    } else {
